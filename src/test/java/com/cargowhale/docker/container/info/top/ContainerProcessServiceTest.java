...
public class ContainerProcessServiceTest {

    @Rule
    public ExpectedException thrown = ExpectedException.none();

    @InjectMocks
    private ContainerProcessService service;

    @Mock
    private ContainerTopClient topClient;
    ...
    public void getContainerProcessesByIdReturnsContainerProcessIndex() {
        ...
        when(this.topClient.containerTop(containerId)).thenReturn(results);
        when(this.processIndexBuilder.buildProcessIndex(containerId, results)).thenReturn(processIndex);

        assertThat(this.service.getContainerProcesses(containerId), is(processIndex));
    }
    ...
    public void throwsContainerNotRunningException_IfContainerIsNotRunning() throws Exception {
        ...
        this.service.getContainerProcesses(containerId);
    }
}