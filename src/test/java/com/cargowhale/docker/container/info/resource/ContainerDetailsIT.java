...
public class ContainerDetailsIT {
    ...
    private static final String CONTAINER_DETAILS_ENDPOINT = "/api/containers/{id}";
    ...
    public void getContainerDetails() throws Exception {
        ...
        this.client.perform(get(CONTAINER_DETAILS_ENDPOINT, CONTAINER_ID).with(TEST_USER_AUTH))
            .andExpect(responseIsInSpec(RamlSpecFiles.CARGO_WHALE_RAML_SPEC_FILE)
                .with(CONTAINER_DETAILS_ENDPOINT, HttpMethod.GET, HttpStatus.OK, MediaTypes.HAL_JSON));

        this.dockerServiceBuilder.reset();
    }

    @Test
    public void getContainerDetails_NoAuthentication() throws Exception {
        this.client.perform(get(CONTAINER_DETAILS_ENDPOINT, CONTAINER_ID))
